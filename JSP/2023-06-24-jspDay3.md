<h1>서블릿</h1>

<h3>서블릿이란 무엇일까</h3>
서블릿(Servlet)은 Java언어을 기반으로 웹 애플리케이션의 동적인 처리를 담당하는 서버 사이트 컴포넌트이다.
클라이언트이 요청을 받아들이고 그에 따라 동적으로 응답을 생성하여 반환하는 역할을 수행한다.
일반적으로 웹 서버에서 호스팅하는 응용프로그램을 확장하는데 사용된다.

잡를 사용하여 웹을 만들기 위해 필요한 기술.
클라이언트가 어더한 요청을하면 그에 대한 결과를 다시 전송해주어야하는데, 이러한 역할을 하는 것
예를 들어, 어떠한 사용자가 로그인을 하려고 할 때, 사용자는 아이디와 비밀번호를 입력하고 로그인 버튼을 누른다.
그때 서버는 클라이언트의 아이디와 비밀번호를 확인한고 다음 페이지를 띄워주어야하는데 이러한 역할을 수해하는 것이 서블릿이다.

서블릿이란? (서버에서 처리해야할 작업들을 담당하는 클래스)<br>
(1) 우리가 처음에 프로젝트 생성시 Dynamic Web Project로 만든 것처럼, 이 클래스도 특별한 클래스 이므로 별도의 메뉴에서 만들어야함
(2) New -> Other -> Web -> Servelt 이라는 것을 찾아서 생성해야함. (어떤 패키지에 소속되어야함.)
(3) 예를 들어서 "TestServlet"이라는 패키지가 있고, 그 안에 TestSErvlet1이라는 클래스를 생성함.
(4) 이 클래스는 우리가 저번주에 한 것 처럼, 일반, jsp페이지에서 접근가능함.(html 페이지도 가능함)

<h3>테스트 서블릿 작업1</h3>
(1) 새로운 프로젝트 생성(TestProject - > 웹 서버셋팅  -> html 페이지 생성(test.html)

(2) test.html페이지는 일단 간단한 기본골격만 있으면 된다.(페이지의 이름은 출력되어야함)<br>
  html페이지는 보통 데이터를 보내는 역할을 함, 여기서도 똑같은 역할을 해줄 것, 그리너 그렇게 보낸 데이터를 지난 시간에는 jsp페이지에서 받았다면 이번에는 중간에 서블릿이 들어가는 것<br>
  3개가 함꺼번에 움직이는 것 처럼, 그러나 한 단계로 가게되면 오류가 굉장히 많이 생기기 때문에 2단계로!

(3) 일단, test.html페이지가 정상적으로 작동되는 것을 확인함

(4)test.html 페이지를 개조한다.<br>
 * 입력상자 1개 + 전송버튼 1개
 * 전송할 곳 "Dummy", 전송방식은 "POST"
어떤 데이터를 박스에 입력했을 때 서블릿이란 클래스로 전송하고 싶은 것.<br>
jsp페이지를 적어주던 자리(action)에 서블릿 클래스 이름을 적어주는 것.<br>
원래 클래스 이름을 저자리에 적는 것이 말이 안되는데 톰캣이 그걸 가능하게 처리해주도록 해주는 것, 이것이 서블릿의 특징 중 하나

(5) test.html(목적지:TestServlet2) --> POST --> 톰캣 --> 서블릿(TestServlet2) --> doPost작동 --> doPost에서 출력명령어 작동함(유저의 웹브라우저로 출력됨)
  * 입력(html) -->처리(서블릿) --> 출력(서블릿) --> 웹브라우저
<br><br>
서블릿을 만들때 보면 superclass가 있는 것을 볼 수 있다.
상속을 받는다는 이야기.
HttpServlet원래 원조 클래스가 있음 그것을 상속받아서 서블릿 클래스를 만드는 것
우리가 jsp페이지에서 처리했던것을 서블릿에서 처리하도록 해주는 것
<br><br>
test.html
```html
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
<form action="TestServlet1" method = "post">
	<h2>Test</h2>
	Data<input type="text" name="data">
	<input type="submit" value="send">
	
</form>
</body>
</html>
```

TestServlet1.java
```java
package TestServletPack;

import jakarta.servlet.ServletException;	//에러처리
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;	//서블릿에 들어오는 입력데이터
import jakarta.servlet.http.HttpServletResponse; //서블릿에서 출력되는 데이터
import java.io.IOException;	//입출력에러 처리
import java.io.PrintWriter;

/**
 * Servlet implementation class TestServlet1
 */
public class TestServlet1 extends HttpServlet {//상속을 받았다.
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TestServlet1() {//생성자
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */ 
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
		
		response.setContentType("text/html");
		//이렇게 아웃프린털 라인에서 코딩을 하게되면 jsp가 조금이라도 커져버리면 곤란해질것, 그래서 jsp가 필요하다.
		PrintWriter out = response.getWriter();
		out.println("<html><body>");
		out.println("<h1>Hello Servelt</h1>");
		out.println(request.getParameter("data"));
		out.println("</body></html>");
		out.close();
	}

}
```
<br><br>

<h3>테스트 서블릿 작업2</h3>
(1) test.html을 개조해서 입력상자 2개를 만든다. (1개는 name = "userid", 1개는 name = "userpw")

(2) 전송버튼을 누르면 2개의 데이터가 자바클래스(서블릿)로 전송된다.

(3) 서블릿에서는 이걸 화면에 출력(브라우저)해주면 된다.

추가적으로 html에서 method를 post로 설정했다면 서블릿에서는 doPost와 연동되어서 작동하고 method를 get로 설정했다면 서블릿에서는 doGet과 연동되어서 작동한다.

test2.html
```html
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Test Servlet2</title>
</head>
<body>
<form method ="post" action ="TestServlet2">
	<input type ="text" name ="userid">
	<br>
	<input type ="text" name ="userpw">
	<br>
	<input type ="submit" value = "send">
</form>
</body>
</html>
```

TestServlet2.java
```java
package testServletPack;

import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.io.PrintWriter;

/**
 * Servlet implementation class TestServlet2
 */
public class TestServlet2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TestServlet2() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
		
		response.setContentType("text/html; charset= UTF-8");
		
		PrintWriter out = response.getWriter();
		out.println("<html><body>");
		out.println("<h1>RESULT</h1>");
		out.println("ID: " + request.getParameter("userid") +"<br>");
		out.println("PW: " + request.getParameter("userpw"));
		out.println("</body></html>");
		out.close();
	}

}
```
<br><br>

<h3>서블릿에서 jsp페이지로 데이터를 전달하는 방법</h3>
(1) request.setAttribute()로 전달하고 싶은 데이터를 저장해둔다.(물건셋팅)<br>
 (박스이름, 저장할 데이터)
  * 전달할 물건은 ("yourid", 아이디)
(2) getRequestDispacher()라는 메소드로 전달할 jsp페이지의 이름을 셋팅한다.
  * test.jsp(전달할 곳)
RequestDispacher은 서블릿에서 다른 리소스(서블릿, JSP, HTML 파일 등)로 요청을 전달하기 위한 인터페이스이다.
getRequestDispatcher()메소드를 사용하여 RequestDispatcher객체를 얻을 수 있다.
다른 리소스로 제어를 전달할 수 있는 두가지 메소드가 있다.
 forward(): 현재의 요청을 다른 리소스로 전달, 다른 리소스에서 생성된 응답을 클라이언트로 직접 전송
  include(): 현재의 요청을 다른 리소스로 포함시킴, 다른 리소스에서 생성된 내용을 현재의 응답에 추가 -> 여러 개의 리소스로부터 생성된 응답이 하나로 조합되어 전송
(3) forward()라는 메소드로 실제로 데이터를 전달(배달)

```java
package testServletPack;

import jakarta.servlet.RequestDispatcher;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.io.PrintWriter;

/**
 * Servlet implementation class TestServlet2
 */
public class TestServlet2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TestServlet2() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		//출력전에 전달해야함
		//전달할 물건 셋팅
		request.setAttribute("yourid", request.getParameter("userid"));
		//전달할 곳(배달주소)
		RequestDispatcher disp = request.getRequestDispatcher("/test.jsp");
		//전달하기
		disp.forward(request, response);
	}
}
```

이렇게 코딩해두면 서블릿이 웹페이지에 뜨는 것이 아니라 jsp페이지가 뜬다.<br>
만약 test.jsp를 이렇게 코딩해두었다면

```jsp
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>test.jsp</title>
</head>
<body>
  <h1>test.jsp</h1>

</body>
</html>
```

test.jsp가 출력되는 화면이 나타날 것이다.
<br><br>
**서블릿을 이요해서 데이터를 처리하는 전체과정**은 다음과 같다.<br>
test.html(데이터 입력, 전송) -> 웹서버(톰캣) -> 서블릿(TestServlet2)
 -> request 객체에서 데이터를 뽑아냄 -> 어떤 박스에 데이터를 셋팅(setAttrbute)
 -> 그 박스를 누군가(test.jsp)에게 전달함(forward) -> 유저에게 전달
<br><br>
MVC패턴(입력-처리-출력)

<br><br>
**와중에 궁금해져서 정리해보는 것들**
request객체는  javax.servlet.http.HttpServletRequest 클래스의 인스턴스입이다.
jsp에서는 이 객체에 대한 미리 정의된 암묵적 객체를 사용할 수 있고 jsp페이지에서 별도의 선언없이 request에 접근할 수 있다.

(1) request.getParameter()메소드를 사용하여 HTTP 요청 매개변수의 값을 읽을 수 있다.
(2) request.getHeader() 메소드를 사용하여 요청 헤더의 값을 읽을 수 있다.
(3) request.getSesssion() 메소드를 사용하여 클라이언트의 상태를 유지하고 데이터를 공유할 수 있다.
(4) request.getAttritube()메소드를 사용하여 요청에 저장된 속성 값을 가져올수 있고
request.setAttritube()메소드를 사용하여 HTTP 요청 객체에 속성을 설장하는 역할을 할 수 있다.
<br><br>

서블릿에서 데이터를 셋팅할 때 사용한 명령어가 중요하다.
다이렉트로 받을땐 request객체의 getparameter()을 사용하면된다. html파일과 jsp가 직접적으로 연관이 되어있을때를 말하는 것
하지만 중간에 서블릿이 있는경우 request객체의 setAttribute() 메소드를 사용함
이것과 반대되는 것이 바로 getAttribute()이다. 속성의 값을 가져올 수 있음
<br><br>
최종적으로 아이디, 비번 두 개 속성을 입력받아 서블릿을 거쳐 jsp페이지에서 출력되게하는 방법

서블릿 코드
```java
package testServletPack;

import jakarta.servlet.RequestDispatcher;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.io.PrintWriter;

/**
 * Servlet implementation class TestServlet2
 */
public class TestServlet2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TestServlet2() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		//전달할 물건 셋팅
		request.setAttribute("yourid", request.getParameter("userid"));
		request.setAttribute("yourpw", request.getParameter("userpw"));
		
		//전달할 곳(배달주소)
		RequestDispatcher disp = request.getRequestDispatcher("/test.jsp");
		//전달하기
		disp.forward(request, response);
	}

}

```

전달받은 값을 출력해주는 jsp 코드
```jsp
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>test.jsp</title>
</head>
<body>
  <h1>test.jsp</h1>
  <%
  String id = (String)request.getAttribute("yourid"); 
  String pw = (String)request.getAttribute("yourpw");
  
  out.println("<h3>" +id +"</h3>");
  out.println("<h3>" +pw +"</h3>");
  %>
</body>
</html>
```

-> 로그인 페이지와 비슷한 형태의 페이지 형태를 만들어봄

여기까지 했을 때 뭐가 문제냐.
아이다와 비번이 전달이 되는데 로그인 되고 나서가 문제임.
로그인이 되었다는 증거를 남겨야하는데 그 증거를 남길 방법이 현재는 없음.
http 프로토콜 특징 상 한 번 접속하고 나면 접속을 끊어버리기 때문에 데이터가 소멸된다.
request랑 response 객체가 소멸된다는 것. 이것을 보완할 수 있는게 session이다.
session을 사용하면user정보를 메모리에 저장할 수 있고 로그인이 되었다 안되었다 증거를 남길 수 있다.

(+) forward와 sendRedirect 차이점
sendRedirect()는 다른 페이지로 이동시켜주는 기능을 함, 페이지 체인지.
forward와 차이점이 있음. forward는 데이터를 받아서 전달해주는 것임.
둘의 차이점이 뭐냐하면 아이디와 비번을 받아서 서블릿에 전달 후 forward해서 jsp에게 주면 데이터 유지가 된다.
그러나 sendReidrect를 사용하게되면 페이지가 체인지되긴하지만 데이터 전달할 수 는 없다.
<br><br>

<h3>html로 보낸 아이디와 암호를 서블릿이 받아서 세션에 저장하고 jsp페이지에서 출력</h3>

**session 객체**

서블릿 코드
```java
package testServletPack;

import jakarta.servlet.RequestDispatcher;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import jakarta.servlet.http.HttpSession;

import java.io.IOException;
import java.io.PrintWriter;

/**
 * Servlet implementation class TestServlet2
 */
public class TestServlet2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TestServlet2() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method s2tub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		//세션이라는 개인사물함을 만들어보자!(쇼핑몰에서 장바구니로 많이 사용)
		HttpSession x = request.getSession();

		x.setAttribute("yourid",request.getParameter("userid"));
		x.setAttribute("yourpw",request.getParameter("userpw"));
		//앞에서 작성한 코드와 다르게 forward가 없기 때문에 페이지 체인지가 이루어지지 않음
		//그래서 redirect가 필요한 것
		
		//response.sendRedirect("/TestProject/test3.jsp");
		response.sendRedirect("test3.jsp");
	}

}
```

jsp 코드
```jsp
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>ID PW Printout test.jsp</title>
</head>
<body>
  <h1>ID PW Printout test.jsp</h1>
  <%
  HttpSession x = request.getSession();
  String id = (String)x.getAttribute("yourid");
  String pw = (String)x.getAttribute("yourpw");
  
  
  out.println("<h3>" +id +"</h3>");
  out.println("<h3>" +pw +"</h3>");
  %>
 
</body>
</html>
```


if문 가미된 버전
```jsp
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>ID PW Printout test.jsp</title>
</head>
<body>
  <h1>ID PW Printout test.jsp</h1>
  <%
  HttpSession x = request.getSession();
  String id = (String)x.getAttribute("yourid");
  String pw = (String)x.getAttribute("yourpw");
  
  out.println("<h3>" +id +"</h3>");
  out.println("<h3>" +pw +"</h3>");
  
  if(id.equals("orieasy1")){
	  
	  if(pw.equals("12345678")){
		  out.println("<center><font color = red><h2>로그인 되었습니다!</h2></font></center>");
	  }else {
		  out.println("비밀번호가 틀렸습니다.");
	  }
  } else {
	  out.println("아이디가 틀렸습니다.");
  }
  
  %>
 
</body>
</html>
```

자바가 버전이 많이 바뀌는 언어이기 때문에 항상 윗부분 버전이 맞는지 확인할 것
<br><br>

<h3>로그인 페이지 제작하기</h3>

(1) 로그인 정보 입력(login.html): 아이디/비번 입력받기
(2) 전송할 곳은 서블릿(loginServlet), 전송방식은 "post"
(3) 서블릿에서 할 일
  * 전달받은 아이디/비번은 세션에 저장(세션의 이름은 자유)
  * 로그인을 처리함(임의의 아이디와 암호를 비교해서 일치하면 로그인된 것으로 간주함)
  * 로그인되었다는 증거를 request.setAttribute() 메소드를 이요해서 저장함
  * forward를 이용해서 login.jsp로 페이지에 전달함
(4) login.jsp페이지에서는 전달받은 결과에 따라서 로그인 되었습니다. 또는 "해당 아이디나 암호가 없습니다"를 출력해줄 것
<br><br>
login.html
```html
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title></title>
</head>
<body>
<form method ="post" action ="LoginServlet">
	ID <input type ="text" name ="userid">
	<br>
	PW <input type ="text" name ="userpw">
	<br>
	<input type ="submit" value ="send">
</form>
</body>
</html>
```

LoginServlet.java
```java
package loginTestPack;

import jakarta.servlet.RequestDispatcher;
import jakarta.servlet.ServletException;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import jakarta.servlet.http.HttpSession;

import java.io.IOException;

/**
 * Servlet implementation class LoginServlet
 */
public class LoginServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public LoginServlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		HttpSession idpw = request.getSession();
		
		idpw.setAttribute("currentid", request.getParameter("userid"));
		idpw.setAttribute("currentpw", request.getParameter("userpw"));
		
		String currentid = (String) idpw.getAttribute("currentid");
		String currentpw = (String) idpw.getAttribute("currentpw");
		
		if(currentid.equals("orieasy1")) {
			if(currentpw.equals("12345678")) {
				request.setAttribute("result", "<h2>로그인되었습니다!</h2>");
			}else {
				request.setAttribute("result", "<h2>해당 아이디나 암호가 존재하지 않습니다.</h2>");
			}
		}else {
			request.setAttribute("result", "<h2>해당 아이디나 비밀번호가 없습니다.</h2>");
		}
		//전달할 곳(배달주소)
		RequestDispatcher send = request.getRequestDispatcher("/login.jsp");
		//전달하기
		send.forward(request, response);
	
	}

}
```

login.jsp
```
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>login.jsp</title>
</head>
<body>
	<h1>로그인 성공여부</h1>
	
	<%
	String result = (String) request.getAttribute("result");
	out.println(result);
	%>
</body>
</html>
```

<h3>세션에 저장된 정보를 다른 페이지(.jsp)에서도 접근가능하지 알아보자!</h3>
  * 세션이라는 것은 삭제되기 전가지는 유지되기 때문에 다른 페이지에서도 접근 가능해야함.

(1) sessionCheck.jsp페이지를 만든다.
(2) 현재 세션에 저장된 아이디와 암호를 출력해본다.
(웹페이지를 만들 경우에 보통 로그인 되어있는지 안했는지 체킹하는 역할을 한다. + 관리자 확인)

sessionCheck.jsp
```jsp
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>sessionCheck</title>
</head>
<body>
	<h1>Session Check</h1>
	<%
	HttpSession idpw = request.getSession();
	String id = (String) idpw.getAttribute("currentid");
	String pw = (String) idpw.getAttribute("currentpw");
	
	out.print("ID: " + id + "<br>PW: " + pw); 
	%>
</body>
</html>
```



